class Page < ActiveRecord::Base
  attr_accessible :active, :partial_name, :title, :filename, :nav_link, :heading, :keywords, :description, :classes
  ...
  def create_starter_block
    Block.new(
      :show_title => true,
      :title => "#{self.title} Page Content", 
      :body => "<h1> #{self.title} </h1> <p>New content.</p>", 
