class UserOld < ActiveRecord::Base
  ...
  has_many :refinance_orders, dependent: :destroy
  has_many :letter_report_orders, dependent: :destroy
  has_many :real_estate_sale_orders, dependent: :destroy

  acts_as_authentic
	
	belongs_to :role
	
	validates_presence_of :proper_name, :address, :phone
	#validates_format_of :username, :format => { :with => /\A[a-zA-Z]+\z/, :message => "" }
